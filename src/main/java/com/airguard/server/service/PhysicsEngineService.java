        ...
        for (Plane plane : activePlanes) {
            ...
            if (plane.getFuel() > 0 && plane.getSpeed() > 0) {
                ...
                if (currentHeading != target) {
                    ...
                    double turnRate = 4.0; // קצב הפנייה: 4 מעלות לשנייה

                    if (Math.abs(diff) <= turnRate) {
                        plane.setHeading(target); // המטוס התיישר
                    } else {
                        plane.setHeading((currentHeading + Math.signum(diff) * turnRate + 360) % 360);
                    }
                }

                // תנועה מדויקת לפי קנה מידה גיאוגרפי
                ...
                // אלגוריתם צריכת הדלק המקורי והחכם
                ...
                // נגמר הדלק - המטוס נופל/נעצר (שלא נדע)
                plane.setSpeed(0);
            }
        }
    }
